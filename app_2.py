...
def clean_biomarker_value(value):
    return None if value == "NaN" else value.split(" ")[0]
...
def predict_next_activity(activity_sequence, feature_values, biomarker_values):
    ...
    feature_array = scaler.transform(feature_df)  # ✅ Ensure correct scaling
...
for i in range(1, len(st.session_state.activity_list)):
    ...
    with col1:
        st.session_state.activity_list[i] = st.selectbox(
            f"Activity {i+1}", all_activities, 
...
for biomarker in biomarker_priority:
    biomarker_values[biomarker] = st.selectbox(f"{biomarker} Level", biomarker_options[biomarker], key=f"biomarker_{biomarker}")

# ✅ Convert Biomarkers for Prediction (Only First Occurrence)
...
if st.button("Predict Next Activity & Time"):
    last_activity = st.session_state.activity_list[-1]  
